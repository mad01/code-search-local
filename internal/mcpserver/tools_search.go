...
func registerSearchTools(s *mcp.Server) {
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Validate a zoekt query and return its parsed tree or a parse error with a fixing hint. " +
			"Use to debug a complex query before invoking csl_search, especially when dealing with escaped regex metacharacters like \\.go$.",
	}, handleQueryValidate)
}
